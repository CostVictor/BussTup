function replace_form(obj_button) {
  ...
  if (
    obj_button.textContent === "Aluno" &&
    !buttons[0].className.includes("selected")
  ) {
    ...
    obj_button.textContent === "Motorista" &&
    !buttons[1].className.includes("selected")
  ) {
  ...
  for (let index = 0; index < list_input.length; index++) {
    ...
    if (input.name === "password_conf") {
      if (input.value.trim() !== data.password) {
        ...
        var erro_text =
          "A senha especificada na confirmação é diferente da senha definida.";
        break;
      }
    } else {
