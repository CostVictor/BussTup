...
function loadInterfaceLine(name_line, load_complete = true) {
    fetch(`/get_interface-line/${encodeURIComponent(name_line)}`, { method: 'GET' })
...
function loadInterfaceDriver(name_line) {
    fetch(`/get_interface-driver/${encodeURIComponent(name_line)}`, { method: 'GET' })
    .then(response => response.json())
    .then(response => {
        if (!response.error) {
...
function loadInterfaceRoutes(name_line) {
    ...
    fetch(`/get_interface-routes/${encodeURIComponent(name_line)}`, { method: 'GET' })
    .then(response => response.json())
    .then(response => {
        if (!response.error) {
...
function config_popup_route(obj_click, data = false) {
    if (!data) {data = return_data_route(obj_click, format_dict_url=true)}
    ...
    .then(response => {
        if (!response.error) {
            ...
            const model_relacao = models.querySelector('#interface_model_option_headli')
            for (nome in data) {
                const tipo = data[nome]
                const paradas = tipo.paradas
                const btn_aba = document.getElementById(`config_route_pontos_container_${nome}_btn`)
                const container = popup.querySelector(`[id*="${nome}_area"]`)
                container.innerHTML = ''
                
                if (return_btn_open(btn_aba)) {action_container(btn_aba)}
                if (role === 'motorista') {
...
function return_data_route(obj_model = false, format_dict_url = false) {
    ...
    if (obj_model) {
        var plate = extract_info(obj_model, 'placa')
        ...
        var plate = document.getElementById('config_route_onibus').textContent
