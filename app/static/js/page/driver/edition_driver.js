function create_line(obj_form, event) {
  ...
  const options_gratuidade = document
    .getElementById("options_gratuidade")
  ...
  for (let index = 0; index < obj_form.length; index++) {
    ...
    if (campo.name) {
      if (
        data.paga &&
        !campo.name.includes("nome") &&
        !campo.name.includes("cidade")
      ) {
        ...
        if (!value || value <= 0) {
          ...
          const nome_campo = `${
            campo.name.includes("cartela") ? "valor_cartela" : "valor_diaria"
          }`;
